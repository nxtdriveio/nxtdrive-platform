-- Sprint 5A follow-up: allow organization admins to manage their own profile.
--
-- The organization profile RPC was introduced for platform-admin screens. The
-- backoffice organization hub uses the same audited RPC, so tenant_admin and
-- franchise_admin users must be able to update their own organization profile
-- without gaining cross-tenant access.
