...
service worker). Only `/student` and `/instructor` are installable surfaces.
...
static, data-free offline page (`/student/offline`, `/instructor/offline`),
...
Run on production (or staging) `https://app.nxtdrive.io`.

### Lighthouse (Chrome DevTools → Lighthouse → PWA), per app
- [ ] `/student` — Installable ✅
- [ ] `/student` — PWA Optimized ✅
- [ ] `/instructor` — Installable ✅
- [ ] `/instructor` — PWA Optimized ✅
...
- [ ] Log in as user A, load `/student`, go offline → still works.
- [ ] Log out, log in as user B (or stay logged out), go offline, open `/student`
...
5. Create two Play listings, upload each `.aab`, fill store listing + Data
   safety, set Privacy policy URL `https://app.nxtdrive.io/privacy`, roll out
