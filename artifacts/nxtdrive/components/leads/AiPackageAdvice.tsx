 ...
 * friendly NL message when AI is not configured or not included in the plan.
 */
export function AiPackageAdvice({
  leadId,
  available = true,
  unavailableReason,
}: {
      ...
      <CardContent className="space-y-4 pt-5">
        ...
        {available ? (
          ...
          <div className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
            {unavailableReason ??
              "AI-functies vereisen het Elite-abonnement."}
          </div>
        )}
        ...
        <p className="flex gap-1.5 text-xs text-muted-foreground">
          ...
          {available
            ? "AI-gegenereerd advies. Controleer en beslis zelf; er wordt niets automatisch toegekend."
            : "Upgrade naar Elite om AI-pakketadvies te gebruiken op basis van intakeprofiel en actieve pakketten."}
        </p>
      </CardContent>
    </Card>
  );
