...
export function BrandingForm({
  ...
  return (
    <div className="space-y-5">
      <form action={saveBranding} className="space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <p className="flex justify-between text-xs text-muted-foreground">
            ...
            <span className="shrink-0 pl-2">{welcomeMessage.length}/120</span>
          </p>
        </div>
      ...
      <form
        action={resetBrandingToNxtdriveDefaults}
        className="rounded-lg border border-border bg-muted/20 px-4 py-3"
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p className="text-sm font-medium text-foreground">
              Terug naar NXTDRIVE standaard
            </p>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              Wist logo, kleuren, welkomsttekst, presets en theme-overrides voor deze tenant.
            </p>
          </div>
          <Button
            type="submit"
