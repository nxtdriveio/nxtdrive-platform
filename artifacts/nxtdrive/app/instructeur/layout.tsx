...
import { loadInstructorLiveCounts } from "@/lib/instructor/counts-server";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    manifest: "/instructeur/manifest.webmanifest",
    appleWebApp: {
      capable: true,
...
export default async function InstructorLayout({
  ...
  if (homePathForRoles(roles) !== "/instructeur") {
  ...
  const [{ items, unreadCount }, liveCounts] = await Promise.all([
    loadInAppNotifications(tenant.id),
    loadInstructorLiveCounts({
      tenantId: tenant.id,
