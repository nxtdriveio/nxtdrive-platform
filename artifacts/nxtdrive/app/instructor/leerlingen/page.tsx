...
export default async function InstructorStudentsPage() {
  ...
  for (const row of lessonRows ?? []) {
    ...
    if (!upcomingLessonMap.has(studentId) && new Date(startsAt).getTime() >= now) {
  ...
  const lowBalanceCount = Array.from(balanceMap.values()).filter((balance) => balance <= 0).length;
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-border/70 bg-background px-3 py-3">
                    ...
                    <p className="mt-1 text-sm font-medium text-foreground">
                      {upcomingLesson ? dateFmt.format(new Date(upcomingLesson)) : "Nog niet ingepland"}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-border/70 bg-background px-3 py-3">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
